...
int main(int argc,char** argv)
{
    ...
    if (inet_pton(AF_INET, server_host, &server.sin_addr) <= 0) {
        // 如果不是合法 IP，则尝试解析为主机名
        hostent* host = gethostbyname(server_host);
        if (host == nullptr) {
            cerr << "gethostbyname failed for " << server_host << endl;
            return -1;
        }
        server.sin_addr = *(struct in_addr*)host->h_addr;
    }

    cout << "Connecting to " << inet_ntoa(server.sin_addr) << ":" << port << endl;
